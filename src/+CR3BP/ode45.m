...
if ~options.STM
  if has_event
    [t,s,te,se,ie] = ode45(@(~,s)CR3BP.ode(s,mu),tspan,[r0;v0],options.options);
  else
    [t,s] = ode45(@(~,s)CR3BP.ode(s,mu),tspan,[r0;v0],options.options);
  end
else
  if has_event
    [t,s,te,se,ie] = ode45(@(~,s)CR3BP.ode_STM(s,mu),tspan, ...
      [r0;v0;reshape(eye(6),[36,1])],options.options);
    se = se(:,1:6);
  else
    [t,s] = ode45(@(~,s)CR3BP.ode_STM(s,mu),tspan, ...
      [r0;v0;reshape(eye(6),[36,1])],options.options);
  end
