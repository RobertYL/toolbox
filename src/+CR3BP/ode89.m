...
if ~options.STM
  s0 = [r0;v0];
  if has_event
    [t,s,te,se,ie] = ode89(@(~,s)CR3BP.ode(s,mu),tspan,s0,options.options);
  else
    [t,s] = ode89(@(~,s)CR3BP.ode(s,mu),tspan,s0,options.options);
  end
else
  s0 = [r0;v0;reshape(eye(6),[36,1])];
  if has_event
    [t,s,te,se,ie] = ode89(@(~,s)CR3BP.ode_STM(s,mu),tspan,s0,options.options);
    se = se(:,1:6);
  else
    [t,s] = ode89(@(~,s)CR3BP.ode_STM(s,mu),tspan,s0,options.options);
  end
